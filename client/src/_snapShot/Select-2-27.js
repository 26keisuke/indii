...
class EditIndexTopic extends Component {
    ...
    componentDidUpdate = (prevProps) => {
        ...
        if(prevProps.index.deleteId !== this.props.index.deleteId) {
            ...
            // for(var j=0; j < copiedPosts.length; j++){
            //     found = arrObjLookUp(newPosts, "_id", copiedPosts[j]._id)
            //     if(!!found){
            //         copiedPosts[j]._id = found
            //     }
            // }
            ...
            console.log(this.state.columns, this.state.posts, this.state.order)
            console.log(copiedColumns, copiedPosts, copiedOrder)
        }
    }
